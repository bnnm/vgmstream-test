...
void replace_filename(char* dst, size_t dstsize, cli_config_t* cfg, libvgmstream_t* vgmstream) {
    ...
    if (subsong > vgmstream->format->subsong_count || subsong != cfg->subsong_index) {
    ...
    do {
        ...
        if (pos[1] == 'n') {
            ...
            /* not recognized */
            // TODO: should move buf or swap "?" with "_"? may happen with non-ascii on Windows; for now break to avoid infinite loops
            break;
        }
...
void print_info(libvgmstream_t* vgmstream, cli_config_t* cfg) {
    ...
    if (!cfg->play_sdtout && !cfg->print_adxencd && !cfg->print_oggenc && !cfg->print_batchvar) {
        ...
        libvgmstream_format_describe(vgmstream, description, 1024);
