...
input_vgmstream::input_vgmstream() {
    ...
    libvgmstream_log_t log = {};
    log.level = LIBVGMSTREAM_LOG_LEVEL_ALL;
    log.callback = log_callback;

    libvgmstream_set_log(&log);
}
...
bool input_vgmstream::decode_run(audio_chunk & p_chunk, abort_callback & p_abort) {
    ...
    switch (vgmstream->format->sample_type) {
...
libvgmstream_t* input_vgmstream::init_vgmstream_foo(t_uint32 p_subsong, const char* const filename, abort_callback& p_abort) {
    ...
    libvgmstream_options_t opt = {0};
    opt.libsf = sf;
    opt.subsong_index = p_subsong;
...
void input_vgmstream::apply_config(libvgmstream_t* vgmstream) {
