...
int sbuf_get_copy_max(sbuf_t* sdst, sbuf_t* ssrc) {
    ...
    if (samples_copy > sdst->samples - sdst->filled)
        samples_copy = sdst->samples - sdst->filled;
    return samples_copy;
}
...
void sbuf_copy_segments(sbuf_t* sdst, sbuf_t* ssrc) {
    ...
    if (ssrc->channels != sdst->channels) {
        int max = sbuf_get_copy_max(sdst, ssrc);
        sbuf_silence_part(sdst, sdst->filled, max);
        sbuf_copy_layers(sdst, ssrc, 0, ssrc->filled);
#if 0
        // "faster" but lots of extra ifs per format, not worth it
        while (src_pos < src_max) {
